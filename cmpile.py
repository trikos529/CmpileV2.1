...
def setup_git_env():
    ...
    if os.path.exists(GIT_CMD):
        # Check if already in path to avoid duplicate
        if GIT_CMD not in os.environ["PATH"]:
            os.environ["PATH"] = GIT_CMD + os.pathsep + os.environ["PATH"]
            return True
    return False

def ensure_environment(log_func=None):
    """Checks and sets up GCC, Git and vcpkg."""
    if log_func: log_func("Checking environment...")
    else: ui.display_status("Checking environment...")
    
    # Check/Install Git first
    download_script.install_git()
    setup_git_env()

    # Check GCC
    if not os.path.exists(GPP_EXE):
        msg = "GCC not found. installing..."
        if log_func: log_func(msg)
        else: ui.display_status(msg)
        try:
            download_script.install_gcc()
        except Exception as e:
            if log_func: log_func(f"Failed to install GCC: {e}", "bold red")
            raise e
    ...
    # Check vcpkg
    vcpkg_mgr = vcpkg_automation.VcpkgManager(INTERNAL_DOWNLOADS)
    if not vcpkg_mgr.is_installed():
        msg = "vcpkg not found. installing..."
        if log_func: log_func(msg)
        else: ui.display_status(msg)
        try:
            download_script.install_vcpkg(git_path_env=GIT_CMD)
        except Exception as e:
            if log_func: log_func(f"Failed to install vcpkg: {e}", "bold red")
            raise e
...
class CmpileBuilder:
    ...
    def build_and_run(self, source_files, compiler_flags=None, clean=False, run=True):
        ...
        if compiler_flags:
            # safe split checking for quotes if possible, or just split
            # shlex.split is better for handling quotes
            try:
        ...
        for src in files:
            ...
            if needs_recompile:
                ...
                try:
                    ...
                    # continue to see other errors? or break? break usually better.
                    break
            else:
        ...
        if run:
            ...
            # For GUI, we might want to capture output or open a console.
            # Capturing output is better.
            ...
            try:
                # If running in GUI, we might want to see output in real time.
                # subprocess.run routes to stdout which we can capture.
                p = subprocess.Popen([output_exe], env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1)
                ...
                while True:
                    ...
                    if not data and p.poll() is not None:
                        break
                    if data:
                        self.log(data.strip()) # Using log callback for program output
                        
                # Also check stderr
                err_data = p.stderr.read()
                if err_data:
                    self.log(err_data, "bold red")
...
if __name__ == "__main__":
    ...
    # For CLI executable
    # Don't pause if invoked by GUI script logic (unlikely here but safety)
